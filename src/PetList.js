...
function PetList() {
    ...
    useEffect(() => {
        const fetchPets = async () => {
            try {
                const response = await axios.get('https://petcare-backend-193c8a8cd9bf.herokuapp.com/api/animals');
    ...
    useEffect(() => {
        const fetchTypes = async () => {
            try {
                const response = await axios.get('https://petcare-backend-193c8a8cd9bf.herokuapp.com/api/types');
    ...
    useEffect(() => {
        const fetchBreeds = async () => {
            if (selectedType !== '') {
                try {
                    const response = await axios.get(`https://petcare-backend-193c8a8cd9bf.herokuapp.com/api/breeds/${selectedType}`);
            ...
            <Row>
                {filteredPets.map(pet => (
                    <Col key={pet.id} md={4}>
                        <PetCard 
                            name={pet.name} 
                            image={`https://petcare-backend-193c8a8cd9bf.herokuapp.com/${pet.file_path}`} 
                            id = {pet.id} 
                        />
                    </Col>
                ))}
