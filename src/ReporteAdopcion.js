  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const [clientResponse, animalResponse] = await Promise.all([
        axios.get(`https://petcare-backend-193c8a8cd9bf.herokuapp.com/api/clients/${formData.user_id}`, {
          headers: {
            Authorization: `Bearer ${authData.token}`,
            Accept: 'application/json'
          }
        }),
        axios.get(`https://petcare-backend-193c8a8cd9bf.herokuapp.com/api/animals/${formData.animal_id}`, {
          headers: {
            Authorization: `Bearer ${authData.token}`,
            Accept: 'application/json'
          }
      ...
      await axios.post('https://petcare-backend-193c8a8cd9bf.herokuapp.com/api/store-adoption-report', formDataToSend, {
        headers: {
          Authorization: `Bearer ${authData.token}`,
          Accept: 'application/json',
