...
function ClientList() {
    ...
    useEffect(() => {
        const fetchClients = async () => {
            ...
            try {
                const response = await axios.get('https://petcare-backend-193c8a8cd9bf.herokuapp.com/api/clients', {
                    headers: {
            ...
            <Row>
                {filteredClients.map(client => (
                    <Col key={client.id} md={4}>
                        <ClientCard 
                            name={client.name} 
                            image={`https://petcare-backend-193c8a8cd9bf.herokuapp.com/${client.image}`} 
                            id={client.id} 
                        />
                    </Col>
                ))}
